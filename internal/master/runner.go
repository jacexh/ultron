...
func (r *MasterRunner) Launch(con ultron.RunnerConfig, opts ...grpc.ServerOption) error {
	...
	if err != nil {
		ultron.Logger.Fatal("failed to launch ultron server", zap.Error(err))
	}
	grpcServer := grpc.NewServer(opts...)
	r.supervisor = newSlaveSupervisor()
	genproto.RegisterUltronAPIServer(grpcServer, r.supervisor)
	ultron.Logger.Info("ultron grpc server is running", zap.String("connect_address", con.GRPCAddr))

	// eventbus初始化
	r.eventbus.SubscribeReport(eventbus.PrintReportToConsole(os.Stdout))
	r.eventbus.Start()
	ultron.Logger.Info("report bus is working")
	...
	err = <-block
	ultron.Logger.Fatal("ultron runner is shutdown", zap.Error(err))
	return err
}
...
func (r *MasterRunner) StartPlan(p ultron.Plan) error {
	...
	if r.plan != nil && r.plan.Status() == ultron.StatusRunning {
		...
		ultron.Logger.Error("failed to start a new plan", zap.Error(err))
		return err
	}
	...
	go func() {
		if err := scheduler.patrol(5 * time.Second); err != nil {
			ultron.Logger.Warn("patrol mission is complete", zap.Error(err))
		}
	}()
	return nil
}
...
func (r *MasterRunner) StopPlan() {
	...
	if err := r.scheduler.stop(false); err != nil {
		ultron.Logger.Error("failed to stop plan", zap.Error(err))
	}
}
